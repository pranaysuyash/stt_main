...
class UserRole(db.Model):
    ...
    user = db.relationship('User', back_populates='user_roles', overlaps="roles,user_roles,users")
    role = db.relationship('Role', back_populates='user_roles', overlaps="users,user_roles,roles")
...
class User(db.Model):
    ...
    user_roles = db.relationship(
        ...
        back_populates='user',
        cascade="all, delete-orphan",
        overlaps="roles,user_roles,users"
    )
    roles = db.relationship(
        'Role',
        secondary='user_roles',
        back_populates='users',
        overlaps="user_roles,roles,users"
    )
    ...
    workspace_memberships = db.relationship(
        ...
        back_populates='user',
        cascade="all, delete-orphan",
        overlaps="workspace_memberships,user"
    )
    subscriptions = db.relationship('Subscription', back_populates='user', cascade="all, delete-orphan")
...
class Role(db.Model):
    ...
    user_roles = db.relationship(
        ...
        back_populates='role',
        cascade="all, delete-orphan",
        overlaps="users,user_roles,roles"
    )
    users = db.relationship(
        'User',
        secondary='user_roles',
        back_populates='roles',
        overlaps="user_roles,users,roles"
    )
...
class WorkspaceMember(db.Model):
    ...
    user = db.relationship('User', back_populates='workspace_memberships', overlaps="workspace_memberships,user")
    workspace = db.relationship('Workspace', back_populates='members')
    role = db.relationship('Role')
...
class OrganizationMember(db.Model):
    ...
    user = db.relationship('User', back_populates='memberships', overlaps="memberships,user")
    organization = db.relationship('Organization', back_populates='members')
    role = db.relationship('Role')
...
#     id = db.Column(db.Integer, primary_key=True)  # Changed from String(36) to Integer
#     user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False)
#     filename = db.Column(db.String(255), nullable=False, index=True)
#     storage_path = db.Column(db.String(500), nullable=False)
#     file_size = db.Column(db.BigInteger, nullable=False)  # File size in bytes
#     file_type = db.Column(db.String(50), nullable=False)  # e.g., 'video/mp4', 'audio/mpeg'
#     duration = db.Column(db.Float)  # Duration in seconds if media file
#     uploaded_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False, index=True)
#     meta_data = db.Column(db.JSON)  # Additional metadata as JSON
#     is_deleted = db.Column(db.Boolean, default=False)
...
#     media_file_id = db.Column(db.Integer, db.ForeignKey('media_files.id'), nullable=False)  # Ensure Integer
